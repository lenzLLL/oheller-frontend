...
import { GET_SELLER_ORDER } from '@/utils/Constant'
            ...
            <div style = {{width:200}} className ="flex justify-left gap-3 items-center" >
               {
                params.row.isActive? <>
                      ...
                      <div  className='py-2 px-3 bg-red-100 rounded-md text-red-600 font-bold cursor-pointer'>Rejeter</div>
                </> :!params.row.isCompleted?<div onClick = {()=>router.push("/seller/orders/message/"+params.row.id)} className='py-2 px-3 bg-blue-100 rounded-md text-blue-600 font-bold cursor-pointer'>Discussion</div>
   ...
   useEffect(
    ...
    return (
        <div className='mt-[120px] flex flex-col px-5 mb-5' >
            <div className='mb-5'>
                <h1 className='font-semibold text-4xl text-[#555]'>Commandes</h1>
            </div>
        <DataGrid
          rows={services}
          columns={columns}
