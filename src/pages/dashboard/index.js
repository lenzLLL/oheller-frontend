...
import DashboardFreelancer from '@/components/admin/freelancer/DashboardSeller'
    ...
    <div className='main min-h-screen bg-[#2a3445]'>
        ...
        <div className='containe flex'>
            <div className='menuContainer w-[250px] py-[5px] px-[20px]  border-r-[2px] border-r-[#384256]'>
                <Menu data = {data} current = {"/dashboard"}/>
            </div>
            <div className='menuContainer py-[5px] px-[20px] w-[100%]'>
                {data.accountType === "Admin" && <DashboardAdmin/>}
                {data?.accountType === "Commerçant (Créez vos Boutiques et vendez vos produits)" && <DashboardSeller/>}
                {data?.accountType === "Freelancer (Faites vous embaucher et gagnez des marchés)" && <DashboardFreelancer/> }
            </div> 
        </div>
        <Footer/>
    </div>
