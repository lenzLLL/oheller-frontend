...
export default function ProductCardStyleOne({ datas,type,rating }) {
  const available =
  ...
  return (
    ...
    >
      <div
        className="product-card-img w-full h-[240px]"
        style={{
          background: `url(${datas.images[0]}) no-repeat`,
      ...
      <div className="product-card-details px-[30px] pb-[30px] relative">
        {/* add to card button */}
        <div className="absolute w-full h-10 px-[30px] left-0 top-[250px] group-hover:top-[85px] transition-all duration-300 ease-in-out">
          <button onClick = {()=>router.push("/product/"+datas.id)} type="button" className={'yellow-btn'}>
            <div  className="flex items-center  space-x-3">
              ...
              <span >{datas.book? "Acheter":"AJouter au panier"}</span>
            </div>
          </button>
        </div>
       <div className ="flex mt-3 gap-1">
       <span
              className={`flex items-center text-[9px] font-700 leading-none py-[6px] pt-[8px] px-3 uppercase text-white rounded-full tracking-wider bg-qyellow
              `}
            >
              {datas.category}
            </span>
            <span
              className={`text-[9px] font-700 leading-none py-[6px] pt-[8px] px-3 uppercase text-white rounded-full tracking-wider bg-[#19CC40]
              `}
            >
              {datas.features[0]}
            </span>
        ...
        <p className="">
          ...
          <span className="offer-price text-qred font-600 text-[18px] mr-2">
            {datas.price} XAF
          </span>
