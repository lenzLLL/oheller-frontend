...
import { ADD_SERVICE, ADD_SHOP, EDIT_SHOP_BY_ID, UPDATE_USER_SERVICE } from '@/utils/Constant';
import { MdDelete } from 'react-icons/md';
import { useRouter } from 'next/router';
export default function DashboardUpdateShop({d}) {
  ...
  const editShop = async () => {
    ...
    try{
        ...
        const response = await axios.put(EDIT_SHOP_BY_ID+"/"+shopId,{numbers,newImages,features,...data},{withCredentials:true})
    ...
    const reader = new FileReader();
      reader.onload = ()=>{
          if(reader.readyState === 2)
          {
              ...
              setData({...data,newCover:reader.result})
  ...
  useEffect(
    ()=>{
    if(d.numbers){
      setData({name:d.name,type:d.type,region:d.region,city:d.city,quarter:d.quarter,description:d.description,secteur:d.secteur,disponibilite:d.disponibilite,cover:d.cover})
      setImages(d.images)
    ...
    return (
        <div className='dashboarshop' >
                   ...
                   <h1 className='text-3xl text-gray-900 mb-5'>Modifiez votre boutique</h1>
       <h3 className='text-xl text-gray-900 mb-5'>Entrez les nouvelles informations pour votre boutique</h3>
         ...
         {option}
           ...
           <div className='flex gap-5 flex-wrap'>
               {
                   numbers.length > 0 && numbers.map(
                    (item)=>{
                      return <span onMouseEnter={()=>setIsHover2(true)} onMouseLeave={()=>setIsHover2(false)} style = {{boxShadow:"inset 0 0 1.5px rgba(0,0,0,0.5)"}} className='px-5 rounded-md py-3 cursor-pointer relative text-[#555]'>
                        {item}
           ...
           <div className='flex gap-5 flex-wrap'>
              ...
              <div className='flex'>
               ...
               { (data.cover||data.newCover) &&              <div className='cursor-pointer flex flex-col m-1 ml-0 bg-gray-50 p-2 w-auto  rounded-md'>
                       <div style = {{borderStyle:"dashed"}} className=' border flex-wrap flex items-center justify-start p-2'>

                                  <img src= {data.newCover?data.newCover:data.cover} className='rounded-md h-[180px] w-[200px] m-1'/>
              ...
              <div className='flex'>
               ...
               { images.length>0 && newImages <= 0 &&              <div className='cursor-pointer flex flex-col m-1 ml-0 bg-gray-50 p-2 w-auto  rounded-md'>
                       <div style = {{borderStyle:"dashed"}} className=' border flex-wrap flex items-center justify-start p-2'>
                           {
                            images.map(
                              (item)=>{
           ...
           <button disabled = {isLoading} onClick={editShop}  className='flex cursor-pointer gap-1 outline-none items-center justify-center border w-[300px] py-[13px] hover:shadow-lg px-20 bg-purple-500 text-white rounded-lg shadow-sm'>
