...
import { ADD_SONDAGE, GET_SONDAGE, UPDATE_SONDAGE_BY_USER } from '@/utils/Constant';
export default function DashboardUpdateQuestion() {
  ...
  const getSondage = async () => {
    try{
        ...
        const data = await axios.get(GET_SONDAGE+sondageId,{withCredentials:true}).then(
            (response)=>{
                let q = []
                let numQuestion = JSON.parse(localStorage.getItem('sondage1-update'))["numQuest"]
                for(let i = 0; i < response.data.questions.length;i++)
                {
                 ...
                 if(numQuestion> response.data.questions.length)
                 {
                      let length = numQuestion - response.data.questions.length
                      for(let i = 0;i<length;i++)
                      {
                        q.push({num:q.length+1,label:"",type:"",label:""})
                      }
                 }
                 else if(numQuestion< response.data.questions.length)
                 {
                      q = q.slice(0,numQuestion)
                 }
  ...
  const goToBack = () => {
    router.push("/dashboard/sondages/sondage-infos-update/"+sondageId)
  }
  const saveSondage = async () => {
    try{
        ...
        const response = await axios.put(UPDATE_SONDAGE_BY_USER+sondageId,{sondage:JSON.parse(localStorage.getItem("sondage1-update")),questions},{withCredentials:true})
  ...
  const isStepSkipped = (step) => {
   ...
   useEffect(
    ...
    return (
       ...
       <h3 className='text-2xl text-gray-900 mb-5 text-center mt-10'>Modifiez les questions pour votre sondage</h3>
